...
- Editing *any* header row (not just row 1) can now change what a `columns=` name reference matches, since the match string spans all header rows — a wider footgun surface than the row-1-only version of this decision, worth a callout in user-facing docs.
