...
- **Blank header cells are skipped when joining.** In the `<br />` join above, a column's empty header-row cells are dropped, the same as for the match string below. So a column reading "Date" over an empty cell shows `Date`, not `Date<br />`.
- **A file shorter than `header-rows` is all header.** `header-rows=3` on a 2-row file makes both rows header rows and leaves zero data rows: a header-only table, not an error. This is consistent with [ADR-0004](0004-table-row-window-options.md), which doesn't treat data-dependent emptiness as an error.
- **Value validation.** The value must be a whole number (digits only); anything else is a directive error, as for the row window options ([ADR-0004](0004-table-row-window-options.md)). An empty value (`header-rows=`) means `1`, the same as the bare form.
