...
- `#markdown` and `#html-table` parse table cells into mdast (`parseCellMarkdown` / `parseCellBlocks`, [cellMarkdown.ts](../../../src/FileInjector/cellMarkdown.ts); see [table-improvements ADR-0008](../table-improvements/0008-table-markdown-cells.md) and [ADR-0010](../table-improvements/0010-table-html-table.md)). The table source is a CSV/TSV file, or a JSON file under `@@inject-table:` ([table-improvements ADR-0011](../table-improvements/0011-table-json-source.md)). A cell like `[spec](spec.md)` in `data/features.csv` was written relative to that file.
...
3. **Markdown table cells are rebased, with the same rules.** Links, images and definitions produced by `#markdown` or `#html-table` cells are rebased relative to the table source file (CSV, TSV or JSON). For a JSON source this covers string values and keys. A nested object or array renders as a code span or code block ([table-improvements ADR-0011](../table-improvements/0011-table-json-source.md) point 5), so it holds no link nodes and is unaffected, consistent with point 1. The same default and the same opt-outs apply (`#rebase-links=false`, `--no-rebase-links`, [ADR-0002](0002-default-on-with-opt-out.md)). Under `#html-table`, definitions are already emitted as literal text ([table-improvements ADR-0010](../table-improvements/0010-table-html-table.md) point 12), so in practice only links and images are affected there. Literal-text tables (no `#markdown`/`#html-table`) have no link nodes and are unaffected.
