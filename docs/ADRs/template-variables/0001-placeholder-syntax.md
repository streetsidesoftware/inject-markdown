...
- The placeholder scanner needs to (a) find and neutralize backslash-escaped occurrences first, then (b) match `\{@\s*([A-Za-z0-9_-]+(?:\.[A-Za-z0-9_-]+)*)\s*@\}` for everything else.
