...
2. **`--values-file [prefix:]path`**, repeatable — one or more JSON files (same JSON-only-for-v1 rule as the directive-level `values-file=`) of run-wide values, each resolved relative to `--cwd`/the injection root rather than to any single document, since it isn't tied to one Markdown file the way a directive's `values-file=` is. Each occurrence takes the same `[prefix:]path` grammar as the directive-level option — bare `path` auto-prefixes from the basename, `prefix:path` sets it explicitly, `:path` merges at the root — per [ADR-0007](0007-values-file-prefixing.md).
...
- **`--values-file` limited to a single path** — the original decision here — reversed alongside the same change to directive-level `values-file=`; see [ADR-0007](0007-values-file-prefixing.md).
...
- `app.mts` needs three new repeatable options — `--value`, `--values-file`, `--allow-env` — each mirrored into `FileInjectorOptions`/`Options`, per this repo's existing convention that every CLI option is registered in both places.
- Repeated `--value name=val` for the same `name`, and repeated `--values-file` entries resolving to the same prefix, both follow the last-wins collision rule from [ADR-0007](0007-values-file-prefixing.md) — the later flag on the command line overrides an earlier one.
