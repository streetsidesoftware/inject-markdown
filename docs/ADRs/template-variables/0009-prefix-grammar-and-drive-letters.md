...
1. **A prefix identifier matches `^(?=.{2,})[A-Za-z0-9_][A-Za-z0-9_-]*(?:\.[A-Za-z0-9_][A-Za-z0-9_-]*)*$`.** The colon is a separator only when the text before it matches; otherwise the whole entry is a path. Read as four rules:
   ...
   This is deliberately stricter than [ADR-0001](0001-placeholder-syntax.md)'s placeholder-name segment (`[A-Za-z0-9_-]+`, which does permit a leading hyphen). A prefix is inferred from, or sits next to, a file path, so it has to be the part that refuses ambiguity; a placeholder name is only ever written on its own.
