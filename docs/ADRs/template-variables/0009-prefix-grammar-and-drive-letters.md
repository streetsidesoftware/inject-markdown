...
1. **A prefix identifier matches `^(?=.{2,})[A-Za-z0-9_][A-Za-z0-9_-]*(?:\.[A-Za-z0-9_][A-Za-z0-9_-]*)*$`.** The colon is a separator only when the text before it matches; otherwise the whole entry is a path. Read as four rules:
   - **Two characters or more.** A Windows drive letter is always exactly one, so it can never be read as a prefix — `C:\data\v.json`, `C:/data/v.json` and `C:package.json` are all paths.
   - **Dot-separated segments, none empty.** `..`, `...`, `a.` and `a..b` are not prefixes; a run of dots can never be mistaken for a namespace.
   - **No leading `-` or `.`,** on the prefix or on any segment: `-foo`, `.env` and `pkg.-x` are paths, not prefixes. A hyphen inside a segment is fine, so `build-info` and `pkg.build-info` are prefixes.
   - **No `/` or `\` anywhere,** so `\\?\C:\data\v.json` and other path-shaped heads are paths for the same reason.

   A segment is therefore exactly [ADR-0001](0001-placeholder-syntax.md)'s placeholder-name segment, `[A-Za-z0-9_][A-Za-z0-9_-]*` — that ADR was tightened to refuse a leading hyphen for this reason, so one rule covers both. A prefix adds only the two-character minimum, which a placeholder name has no need of: a name is written on its own, never next to a path.
...
- **`[A-Za-z0-9._-]{2,}` as a flat character class** — the first form of point 1 — rejected: it admits `..`, `...`, `.env` and `-foo` as prefixes, so a run of dots or a dotfile-shaped path could be read as a namespace. Point 1 spells out the segment structure instead of a bare class.
