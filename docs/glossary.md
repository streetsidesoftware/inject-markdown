...
Rewriting a path-relative URL in injected Markdown so it resolves from the host file the same way it resolved from the source file. Applies to `link`, `image`, and `definition` nodes. Absolute, protocol-relative (`//`), root-relative (`/`), and fragment-only (`#`) URLs, and raw HTML attributes, are left untouched. See [relative-links/ADR-0001](ADRs/relative-links/0001-rebase-scope.md) and [ADR-0003](ADRs/relative-links/0003-rebase-base-resolution.md).
